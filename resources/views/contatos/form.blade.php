            ...
            <div class="grid grid-cols-1 gap-2">
                <div class="grid grid-cols-2">
                    <div>
                        ...
                        <label for="nome">
                            <p class="text-lg text-[#434955] font-bold">Nome:</p>
                        </label>
                        <input placeholder="Nome" name="nome" id="nome"
                            class="button  focus:outline-[#556787] hover:border-[#556787]  hover:shadow-lg bg-white border-2 border-[#85A0D4] col-span-2 rounded-lg text-black px-6 py-3 text-base  cursor-pointer transition"
                ...
                <div id="telefoneDiv" class="grid gap-2">
                    <label for="">
                        <p class="text-lg text-[#434955] font-bold ">Telefone: </p>
                    </label>
                    @if (isset($contato))
                        @foreach ($contato->telefoneNumero as $telefone)
                            <div class="">
                                ...
                                <input
                                    class="button focus:outline-[#556787] hover:border-[#556787] hover:shadow-lg bg-white border-2 border-[#85A0D4] rounded-lg text-black px-6 py-3 text-base  cursor-pointer transition"
                                    type="text" name="telefoneNumero[]" value="{{ $telefone->numero }}" required>
                                @if (!$loop->first)
                                    <button type="button" onclick="removeTelefone(this)"
                                        class="button hover:shadow-lg shadow-red-500/50 hover:shadow-red-500/100 bg-[#F5533B] border-red-400 hover:border-red-500 text-white font-bold py-2 px-4">
                                        <p class="text-xs font-semibold">Remover</p>
                                    </button>
                                @endif
                            </div>
                        @endforeach
                ...
                <div class="">
                    <button
                        class="bg-[#3B82F6] border-2 border-blue-500 rounded-lg ms-2 text-white  text-base  shadow-blue-500 shadow-md hover:shadow-blue-700 hover:shadow-md cursor-pointer transition"
                        id="addTelefone" type="button" role="button">
                        <p class="text-base font-semibold p-2">Adicionar Número</p>
                    </button>
                </div>

                <p class="text-lg text-[#434955] font-bold mt-2">Endereço:</p>
                <div class="grid grid-cols-2 gap-2 gap-x-14 ">
                    <div>
                        <label for="cidade">
                            <p class="text-base text-[#3F5376] font-semibold mb-1">Cidade: </p>
                        </label>
                        <input
                            class="button focus:outline-[#556787] hover:border-[#556787] hover:shadow-lg bg-white border-2 border-[#85A0D4] rounded-lg text-black w-32 md:w-64  px-6 py-3 text-base  cursor-pointer transition"
                            type="text" placeholder="Cidade" name="cidade" id="cidade"
                            value="{{ isset($contato) ? $contato->endereco->cidade : '' }}" required>
                    </div>
                    <div>
                        <label for="numero">
                            <p class="text-base text-[#3F5376] font-semibold mb-1 ms-8">N&#176;:</p>
                        </label>
                        <input
                            class="button focus:outline-[#556787] hover:border-[#556787] hover:shadow-lg bg-white border-2 border-[#85A0D4] rounded-lg text-black w-16 md:w-24 ms-8 px-6 py-3 text-base  cursor-pointer transition"
                            placeholder="N&#176; da Casa" type="text" name="numero" id="numero"
                            value="{{ isset($contato) ? $contato->endereco->numero : '' }}" required>
                    </div>
                    <div>
                        <label for="rua">
                            <p class="text-base font-semibold mb-1 text-[#3F5376] ">Rua: </p>
                        </label>
                        <input
                            class="button focus:outline-[#556787] hover:border-[#556787] hover:shadow-lg bg-white border-2 border-[#85A0D4] rounded-lg text-black w-64 px-6 py-3 text-base  cursor-pointer transition"
                ...
                <div class="flex justify-center gap-2 mt-2">
                    <button class="button hover:shadow-xl bg-[#809ACB]" type="submit">
                        <p class="text-base font-bold "> Salvar</p>
                    </button>
                    <button class="button hover:shadow-xl bg-[#809ACB] " type="button"
                        onclick="event.preventDefault(); window.location.href='/index'">
                        <p class="text-base font-bold ">Cancelar</p>
                    </button>
                </div>
